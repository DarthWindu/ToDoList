...
public class HistoryWindow implements MouseListener{
	...
	HistoryWindow(Task task, Scene scene) {
		...
		//new Tool().setStageListener(stage);
		stage.showAndWait();
	}
	...
	private void update() {
									...
									if(comm.getComment().equals(withdraw)) {
										...
										if ((clickCounter % 2) != 0) {
											...
											Platform.runLater(new Runnable() {
												@Override
												public void run() {
													new FXCommentWindow(t,comm, mystage);
													System.out.println("fall through");
													//update();
													SwingUtilities.invokeLater(new Runnable() {
														@Override
		...
		createSwingContent(swingN);
		pane.setContent(swingN);
		mystage.setScene(new Scene(pane, 400, 850));
		mystage.setTitle("History window");
		/*stage.initModality(Modality.WINDOW_MODAL);
		stage.initOwner(
				(sc.getWindow()));*/
		mystage.show();
	...
	public void refresh(Task task, Scene scene) {
		history = task.getHistoryItems();
		String text = "";
		t = task;
		sc = scene;
